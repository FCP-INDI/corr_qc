...
setwd("~/zarrar/qc")
df <- read.csv("corr.qc/qc_filt.csv")[,-1]
df <- df[,colnames(df)!="fd"] # temporary
...
#+ setup-percentile
qc.measures <- colnames(df)[!(colnames(df) %in% c("subject", "site", "site.name", "session", "scan", "global"))]
...
mdf <- data.frame(
    measure = qc.measures, 
    description = c(
        "Foreground to Background Ratio (FBER)", 
...
#' ## QC Measures
#' I will be plotting a bunch of different FBER data-sets here. Actually really just three options.
#+ plot-qc, fig.width=10, fig.height=10
for (i in 1:nrow(mdf)) {
